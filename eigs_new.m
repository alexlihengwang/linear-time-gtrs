% eigs_new.m
% modified from the MATLAB buit-in function eigs.m
% no changes are made to the algorithms, only made changes to loag and output iterates.

function [V, D, out] = eigs_new(varargin)
...
[V, d, isNotConverged, spdBout, VV, info] = KrylovSchur(applyOP, applyM, innerOpts, n, k,...
    shiftAndInvert, randStr, spdB);

tic;
% Do some post processing of the output for generalized problem
fprintf('\t\tpostProcessing\n');
[V, d, isNotConverged, V_log, d_log] = postProcessing(V, d, isNotConverged, eigsSigma, B, scaleB, ...
    shiftAndInvert, R, permB, spdB, nargout, info);
...
out.V_log = V_log;
out.d_log = d_log;
out.time = t1 + info.time + t3;
out.time_log = t1 + info.time_log;
end
...
function [V, d, isNotConverged, spdB, VV, info] = KrylovSchur(applyOP, applyM, ...
    innerOpts, n, k, shiftAndInvert, randStr, spdB)

tic;
fprintf('\t\tCalling KrylovSchur \n');
...
if innerOpts.ishermprob
    ...
    [V, d, isNotConverged, stopAlgorithm, VV, info_1] = KSnonherm(applyOP, applyM, n, k, ...
        innerOpts, randStr, shiftAndInvert);
end

tic;
% fprintf('\tstopAlgorithm = %d \n', stopAlgorithm);
...
if stopAlgorithm
    if spdB && shiftAndInvert
        ...
        applyM = @(x) x;
        [V, d, isNotConverged, stopAlgorithm, VV, info_2] = KSnonherm(applyOP, applyM, n, k, ...
            innerOpts, randStr, shiftAndInvert);
    end
...
t3 = toc;
info = info_1;
info.time = t1 + info_1.time + t3;
info.time_log = t1 + info_1.time_log;
end
...
function [U, d, isNotConverged, stopAlgorithm, V, info] = KSnonherm(applyOP, applyM, n, k, ...
    innerOpts, randStr, shiftAndInvert)
% Applies Krylov Schur algorithm for non-Hermitian matrices (reference 1)

tic;
fprintf('\t\tCalling KSnonherm\n')
info = struct();
...
function [V, d, isNotConverged, V_log, d_log] = postProcessing(V, d, isNotConverged, eigsSigma, B, scaleB, shiftAndInvert, R, ...
    permB, spdB, nOutputs, info)
